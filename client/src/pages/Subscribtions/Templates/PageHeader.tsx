...
function FilterButtons({ isMobile }: { isMobile: boolean }) {
  const titleFilter = useAppSelector(
    (state) => state.subscribtionsPageReducer.templates.filters.title,
  );
  const statusFilter = useAppSelector(
    (state) => state.subscribtionsPageReducer.templates.filters.status,
  );
...
export function SubscribtionsTemplatesHeader() {
